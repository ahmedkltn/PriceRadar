...
class ProductDetailView(APIView):
    ...
    def get(self, request, product_id):
        ...
        reviews_qs = Review.objects.filter(product_id=product.product_id, is_active=True)
        total_reviews = reviews_qs.count()
        average_rating = reviews_qs.aggregate(Avg('rating'))['rating__avg']
        
        # Rating distribution (count of reviews per rating 1-5)
        rating_distribution = {}
        for rating in range(1, 6):
            rating_distribution[rating] = reviews_qs.filter(rating=rating).count()
